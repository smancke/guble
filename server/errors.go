...
var (
	...
	// ErrInvalidRoute is returned by the `Deliver` method of a `Route` when it has been closed due to slow processing
	ErrInvalidRoute = errors.New("Route is invalid. Channel is closed.")

	// ErrChannelFull is returned when trying to `Deliver` a message with a queue size of zero and the channel is full
