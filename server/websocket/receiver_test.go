...
func Test_Receiver_Fetch_Subscribe_Fetch_Subscribe(t *testing.T) {
	...
	subscribe := routerMock.EXPECT().Subscribe(gomock.Any()).Do(func(r *server.Route) {
		...
		r.Messages() <- server.MsgAndRoute{Message: &protocol.Message{ID: uint64(4), Body: []byte("router-a"), Time: 1405544146}, Route: r}
		r.Messages() <- server.MsgAndRoute{Message: &protocol.Message{ID: uint64(5), Body: []byte("router-b"), Time: 1405544146}, Route: r}
